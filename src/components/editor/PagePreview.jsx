...
export default function PagePreview({ page }) {
  ...
  const style = {}
  Object.entries(theme.vars).forEach(([k, v]) => { style[k] = v })

  return (
    <div style={{ ...style, backgroundColor: 'var(--bg)', color: 'var(--text)', fontFamily: 'var(--font-body)' }}>
      {/* Hero */}
      <section style={{ backgroundColor: 'var(--bg)', padding: '5rem 2rem', textAlign: 'center' }}>
        <h1 style={{
      ...
      {/* Features */}
      <section style={{ backgroundColor: 'var(--surface)', padding: '4rem 2rem' }}>
        <h2 style={{ fontFamily: 'var(--font-heading)', textAlign: 'center', fontSize: '2rem', fontWeight: 700, marginBottom: '3rem' }}>
          Features
        </h2>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.5rem', maxWidth: '1000px', margin: '0 auto' }}>
          {content.features.map((f, i) => (
            <div key={i} style={{
              ...
              <h3 style={{ fontWeight: 700, marginBottom: '0.5rem', color: 'var(--accent)' }}>{f.title}</h3>
      ...
      {/* Gallery */}
      <section style={{ backgroundColor: 'var(--bg)', padding: '4rem 2rem' }}>
        <h2 style={{ fontFamily: 'var(--font-heading)', textAlign: 'center', fontSize: '2rem', fontWeight: 700, marginBottom: '3rem' }}>
          Gallery
        </h2>
      ...
      <section style={{ backgroundColor: 'var(--surface)', padding: '4rem 2rem' }}>
        <div style={{ maxWidth: '500px', margin: '0 auto', textAlign: 'center' }}>
          ...
          <p style={{ color: 'var(--text-muted)', marginBottom: '2rem' }}>{content.contact.subheading}</p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            {['Name', 'Email'].map(f => (
              <input key={f} placeholder={f} disabled style={{
            ...
            <textarea placeholder="Message" disabled rows={4} style={{
              padding: '0.75rem 1rem',
              borderRadius: 'var(--radius)',
              border: '1px solid var(--border)',
              backgroundColor: 'var(--bg)',
